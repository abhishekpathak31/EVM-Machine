...
ADCON1=0x0F;
 ...
 c3k=firstchar(dat_can3);
   Lcd_Out(1,1,"C_1 C_2 C_3");
     ...
     Lcd_Chr(2,12,'*');
     Lcd_Chr(2,13,'*');
  if(porta==0b00001000){  // if control button is pressed , it enables voting.
  con=3;
 }
 if(con!=3){    // if control button is not pressed ,naturally it disables voting.
 Lcd_Out(2,15,"DV");
 PORTC.C0=0;
 portc==0b00000010;     //LED2 is on
 }
 while(con==3){    // if control button is pressed , it enables voting.
  Lcd_Out(2,15,"EV");
 (portc==0b00000001;     //LED1 is on
 PORTC.C1=0;
 if(portc==0b00000100){   // when view status button is pressed
     Lcd_Chr(2,1,c1i);
     Lcd_Chr(2,2,c1j);
     Lcd_Chr(2,3,c1k);
     Lcd_Chr(2,6,c2i);
   ...
   if(portd==0b00001000){      // If Result Button is pressed .
    Lcd_Cmd(_LCD_CLEAR);
    for(j=1;j<17;j++){
